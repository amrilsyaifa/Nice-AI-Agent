...
def config_set(key: str, value: str):
    """Set a config value. Example: nice config set provider openai"""
    config = load_config()
    ...
    if key == "provider":
        ...
        typer.echo(f"Unknown key '{key}'. Available: provider, model, api_key, base_url")
        raise typer.Exit(1)
...
def config_get(key: str):
    """Get a config value. Example: nice config get provider"""
    config = load_config()
